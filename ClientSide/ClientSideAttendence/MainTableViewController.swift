...
class MainTableViewController: UITableViewController, MCSessionDelegate, MCBrowserViewControllerDelegate {
    ...
    @IBAction func extraItemsAction(_ sender: Any) {
        ...
        dropDown.dataSource = ["Add Classroom", "Log Out"]
        dropDown.show()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        fstore.collection("teacher").document(Auth.auth().currentUser!.uid).getDocument { (document, error) in
            if let documents = document {
                if let firebaseTeacher : teacher = try! FirestoreDecoder().decode(teacher.self,from: documents.data()!) {
                    teacherList.myTeacher = firebaseTeacher
                    print("data loaded")
    ...
    override func viewDidLoad() {
        ...
        myrefreshControl.addTarget(self, action: #selector(userReloadTableView(_:)), for: .valueChanged)
        /////
        fstore.collection("teacher").document(Auth.auth().currentUser!.uid).getDocument { (document, error) in
            if let documents = document {
                if let firebaseTeacher : teacher = try! FirestoreDecoder().decode(teacher.self,from: documents.data()!) {
    ...
    @objc private func userReloadTableView(_ sender: Any) {
        fstore.collection("teacher").document(Auth.auth().currentUser!.uid).getDocument { (document, error) in
            if let documents = document {
                if let firebaseTeacher : teacher = try! FirestoreDecoder().decode(teacher.self,from: documents.data()!) {
                    teacherList.myTeacher = firebaseTeacher
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return teacherList.myTeacher.classList.count
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.courseName.text = teacherList.myTeacher.classList[indexPath.row].Classname
        cell?.courseCode.text = teacherList.myTeacher.classList[indexPath.row].courseCode
        cell?.studentCount.text = String(teacherList.myTeacher.classList[indexPath.row].countStudents()) + " Students"
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        teacherList.myTeacher.classList.remove(at: indexPath.row)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "addClassroom" {
            if let myAddCourseViewController = segue.destination as? AddClassroomTableViewController {
                myAddCourseViewController.teacher = teacherList.myTeacher
                ...
                if let myClassroomDetailViewController = segue.destination as? StudentTableViewController {
                    ...
                    if let indexPath = indexOfCell {
                        print("guess so")
                        let classroomToSend = teacherList.myTeacher.classList[indexPath]
...
extension MainTableViewController: addCourseViewControllerDelegate {
    ...
    func addCourseViewController(_ controller: AddClassroomTableViewController, didFinishAdding item: classroom) {
        teacherList.myTeacher.classList.append(item)
        print("try to exit")
        myTeacherList.updateData()
        navigationController?.popViewController(animated: true)
    }
